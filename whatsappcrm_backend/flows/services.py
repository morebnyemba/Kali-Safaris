...
def _handle_fallback(current_step: FlowStep, contact: Contact, flow_context: dict, contact_flow_state: ContactFlowState) -> List[Dict[str, Any]]:
    ...
    if current_step.step_type == 'question':
        ...
        if fallback_config.action == 're_prompt' and current_fallback_count < max_retries:
            ...
            else: # ROBUSTNESS: Default behavior after retries is now human handover, not silent exit.
    ...
    for field_path, value_template in fields_to_update_config.items():
        ...
        if len(parts) == 1: # Direct attribute on CustomerProfile model
            ...
            if current_level is not None:
                ...
                elif isinstance(current_level, list): # Should not happen for dict-based JSONFields like custom_attributes
        ...
        while True:
            ...
            if current_step.step_type == 'question' and '_question_awaiting_reply_for' in flow_context:
                ...
                # Handle resumption from WhatsApp Flow response that was already saved to context
                    ...
                    if flow_context.get('whatsapp_flow_response_received') and flow_context.get('whatsapp_flow_data'):
                        nfm_response_data = flow_context.get('whatsapp_flow_data')
...
def process_whatsapp_flow_response(msg_data: dict, contact: Contact, app_config) -> tuple[bool, str]:
    ...
    business entities (e.g., TourInquiry, Booking).
    ...
    try:
        ...
        if whatsapp_flows.exists():
            ...
            # This logic can be adapted for tour inquiries
            if 'destinations' in response_fields and 'preferred_dates' in response_fields: # Example for a Tour Inquiry Flow
                whatsapp_flow = whatsapp_flows.filter(name='tour_inquiry_whatsapp').first()
            
            if not whatsapp_flow:
                # Fallback to the first active flow if no specific match, or handle as an error
                logger.warning(f"Could not determine specific WhatsApp flow for response. Falling back to first available.")
