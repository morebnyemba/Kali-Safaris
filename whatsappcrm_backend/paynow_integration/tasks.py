...
def send_payment_confirmation_task(payment_id: str):
    ...
    try:
        ...
        from pdfs.services import generate_receipt_pdf
        pdf_url = generate_receipt_pdf(payment)
        if pdf_url:
            receipt_caption = f"Here is your official receipt for booking {payment.booking.booking_reference}."
            pdf_message_data = {
                "link": pdf_url,
                "filename": f"Receipt_{payment.booking.booking_reference}.pdf",
                "caption": receipt_caption
            }
            # Use the active config to send the document
            active_config = get_active_meta_config_for_sending()
            send_whatsapp_message(to_phone_number=contact_to_notify.whatsapp_id, message_type='document', data=pdf_message_data, config=active_config)
            logger.info(f"{log_prefix} Successfully sent PDF receipt to user {contact_to_notify.whatsapp_id}.")
        else:
        ...
        with transaction.atomic():
            ...
            if status_response['success']:
                ...
                if paynow_status == 'paid':
                    ...
                    if pending_payment.booking:
                        pending_payment.booking.payment_status = Booking.PaymentStatus.PAID
                        pending_payment.booking.save(update_fields=['payment_status', 'updated_at'])
                        logger.info(f"{log_prefix} Updated booking {pending_payment.booking.id} payment_status to PAID.")
    ...
    try:
        with transaction.atomic():
            ...
            if paynow_status == 'paid':
                ...
                # --- NEW: Update associated order status ---
                if payment_to_update.booking:
                    payment_to_update.booking.payment_status = Booking.PaymentStatus.PAID
                    payment_to_update.booking.save(update_fields=['payment_status', 'updated_at'])
